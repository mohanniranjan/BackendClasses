...
const upadteEmp=async (req,res)=>{
    ...
    const response= await Employee.findOneAndUpdate({name:name},{age:age},{new:true})
    if(response){
        res.json({
...
const updateEmployees=async (req,res)=>{
    const {age}=req.body
    const response=await Employee.updateMany({age:{$eq:30}},{age:age})
    if(response){
        res.json({
...
module.exports = { create, createMany, getAllEmp, getEmployee,upadteEmp,updateEmployees ,deleteEmp};
